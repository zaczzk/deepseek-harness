...
会话总量对 `tokenUsageByModel` 的路由行求和（助手结算与 compaction 摘要调用）。项目总量对当前会话所属工作区的每个会话求和，其他会话取会话列表携带的缓存投影值。额度行是提供方报告窗口的整数百分比，仅横条封顶显示；没有归属声明的路由使用共享的 `unknown` 文案。延迟数字对当前模型的 `modelLatency` 样本在最近 15 分钟与 1 小时窗口求平均——只有真实发生的调用参与，因此空闲的模型既不显示数字、也不稀释结果。
