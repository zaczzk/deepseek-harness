...
面板按固定顺序列出各行——`Session`、`Project`、`Latency 15m`、`Latency 1h`、`Month`、`Compensation`、`Plan`、`Resets`——随后是本会话的计费提供方/模型路由（当前模型高亮）。每个行值带有悬停与聚焦显示的复制控件（触控指针下常驻）。延迟行走同一套阶梯：不足 5 个样本只显示均值，5 个起显示 `1.2s · p95 3.1s`，含首 token 的调用满 5 个起显示 `1.2s · p95 3.1s · ttft 240ms`（`ttft` 取其均值）。会话总量对 `tokenUsageByModel` 的路由行求和（助手结算与 compaction 摘要调用）。项目总量对当前会话所属工作区的每个会话求和，其他会话取会话列表携带的缓存投影值。控制台会话过期时，行区整体替换为一行可操作提示，胶囊上的额度数字随之隐去。
